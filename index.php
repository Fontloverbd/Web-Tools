...
function handleMessage($message) {
    ...
    // FIX: Forcefully handle global commands first and clear state so bot never gets stuck in conversion input mode
    if ($trimmed === '/start' || $text === '/start' || $trimmed === BTN_HOME) {
        clearState($userId);
...
// Gemini API Code Conversion Function (Optimized & Robust)
function convertCodeWithGemini($code, $targetLang) {
    if (!defined('GEMINI_API_KEY') || GEMINI_API_KEY === 'YOUR_GEMINI_API_KEY_HERE' || empty(GEMINI_API_KEY)) {
        return "❌ Gemini API Key সেট করা হয়নি! কোডের কনফিগারেশনে সঠিক API Key বসান।";
    }
    ...
    $prompt = "You are an expert programming assistant. Convert the provided source code completely and accurately into {$targetLang}. Return ONLY the converted code inside a plain text format. Do not include markdown code ticks if possible, or ensure it is clean and directly executable.";
    ...
    if (isset($decoded['candidates'][0]['content']['parts'][0]['text'])) {
        $convertedText = $decoded['candidates'][0]['content']['parts'][0]['text'];
        // Remove markdown code blocks wrapper if returned by Gemini
        $convertedText = preg_replace('/^```[a-z]*\s*\n?/i', '', $convertedText);$convertedText = preg_replace('/\n?```\s*$/', '', $convertedText);
        return trim($convertedText);
    }
