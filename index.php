...
function getFileContentFromTelegram($fileId) {
...
function convertCodeHelpText() {
    return "📚 <b>Convert Code — সম্পূর্ণ গাইডলাইন</b>\n\n" .
        ...
        "<b>ধাপ ২:</b> আপনার কোডের ফাইল (.txt বা .php) আপলোড করুন।\n" .
        "<b>ধাপ ৩:</b> ফাইল আপলোড হওয়ার পর নিচের **🚀 Run Convert** বাটনে ক্লিক করুন।\n\n" .
        "🏠 মূল মেনুতে ফিরতে চাইলে Home বাটনে চাপুন।";
}
...
function handleMessage($message) {
    ...
    // Global Commands
    if ($rawText === '/start' || $rawText === BTN_HOME) {
        clearState($userId);
    ...
    switch ($trimmed) {
        ...
        case BTN_RUN_CONVERT:
            if (!empty($state['convert_parts'])) {
                ...
                sendMessage($chatId, "❌ কোনো কোড ফাইল বা ডেটা পাওয়া যায়নি! প্রথমে কোড ফাইল আপলোড করুন।", convertActionKeyboard());
            }
            return;
        case BTN_CLEAR_CONVERT:
            $state['convert_parts'] = [];
            saveState($userId, $state);
            sendMessage($chatId, "🗑 কনভার্ট করার কোড বাফার পরিষ্কার করা হয়েছে। নতুন কোড ফাইল পাঠান:", convertActionKeyboard());
            return;
        case BTN_CODE_SUBMIT:
            $state['mode'] = 'code_submit';
            $state['code_buffer'] = [];
            saveState($userId, $state);
            sendMessage($chatId, "📝 কোড ফাইল বা টেক্সট পাঠান। শেষ হলে 🎨 Create Color চাপুন।", buttonColorMenuKeyboard());
            return;
        ...
        case BTN_HELPLINE:
            if ($state['menu'] === 'button_color') {
                sendMessage($chatId, buttonColorHelpTest(), buttonColorMenuKeyboard());
        ...
        case BTN_SINGLE_MODE:
            ...
            saveState($userId, $state);
            sendMessage($chatId, "📝 কোড ফাইল বা টেক্সট পাঠান। শেষ হলে 📦 Create File চাপুন।", codeToFileMenuKeyboard());
            return;
        case BTN_MULTI_MODE:
    ...
    if ($state['menu'] === 'convert_code' && $state['mode'] === 'select_lang') {
        ...
        if (isset($langs[$trimmed])) {
            ...
            $state['convert_parts'] = [];
            saveState($userId, $state);
            sendMessage($chatId, "✅ আপনি সিলেক্ট করেছেন: <b>{$langs[$trimmed]}</b>\n\nএখন আপনার কোডের ফাইল (.txt বা .php) আপলোড করুন অথবা কোড টেক্সট আকারে পাঠান। ফাইল পাঠানো শেষ হলে নিচের <b>🚀 Run Convert</b> বাটনে চাপুন।", convertActionKeyboard());
            return;
        }
    }
    ...
    $contentToAdd = null;
    if ($document !== null) {
        $contentToAdd = getFileContentFromTelegram($document['file_id']);
    } elseif ($rawText !== null) {
        $contentToAdd = reconstructRawText($rawText, $message['entities'] ?? []);
    }

    if ($contentToAdd !== null && $contentToAdd !== '') {
        if ($state['mode'] === 'code_submit') {
            $state['code_buffer'][] = $contentToAdd;
            saveState($userId, $state);
            $partCount = count($state['code_buffer']);
            sendMessage($chatId, "✅ কোড/ফাইল অংশ যোগ হয়েছে। (মোট অংশ: <b>{$partCount}</b>টি)", buttonColorMenuKeyboard());
            return;
        }
        if ($state['mode'] === 'single_mode') {
            $state['single_code_parts'][] = $contentToAdd;
            saveState($userId, $state);
            $partCount = count($state['single_code_parts']);
            sendMessage($chatId, "✅ কোড/ফাইল অংশ যোগ হয়েছে। (মোট অংশ: <b>{$partCount}</b>টি)", codeToFileMenuKeyboard());
            return;
        }
        if ($state['mode'] === 'multi_mode') {
            $state['multi_parts'][] = $contentToAdd;
            saveState($userId, $state);
        ...
        if ($state['mode'] === 'convert_input') {
            $state['convert_parts'][] = $contentToAdd;
            saveState($userId, $state);
            $partCount = count($state['convert_parts']);
            sendMessage($chatId, "✅ কোড ফাইল/অংশ সফলভাবে গৃহীত হয়েছে। (মোট অংশ: <b>{$partCount}</b>টি)\nআরও ফাইল থাকলে পাঠান অথবা কনভার্ট করতে নিচের <b>🚀 Run Convert</b> বাটনে চাপুন।", convertActionKeyboard());
            return;
        }
    }
}
