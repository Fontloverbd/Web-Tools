...
// ============================================================
/**
 * Fix (v2): key order independence + arrow/colon consistency.
 *
 * Previously this function only looked *forward* from the 'text'
 * key for a button-hint key (url, callback_data, ...). If the
 * submitted code declared the hint key BEFORE 'text' (very common,
 * e.g. ['url' => $url, 'text' => 'Visit']), the hint was never seen
 * and no style was injected — meaning the bot silently did nothing
 * for a large share of real submissions.
 *
 * Now: for every 'text' key found, we locate the *entire* enclosing
 * array/argument-list it lives in (via bracket matching), and scan
 * ALL sibling keys inside that same bracket — before and after
 * 'text' — for a button hint. We also reuse the actual arrow style
 * (=> or :) found at the 'text' key when inserting the new "style"
 * field, so mixed named-argument syntax isn't broken.
 */
function applyButtonStyles($code) {
    if (!function_exists('token_get_all')) { return $code; }

    $trimmedCode = ltrim($code);
    $hasPhpTag = (strpos($trimmedCode, '<?') === 0);
    ...
    // ---- Step 1: match every bracket pair ([ ], { }, ( )) and record,
    //      for every token, which bracket directly encloses it ----
    $matchOf = [];      // open index => close index
    $enclosingOf = [];  // token index => index of innermost open bracket containing it (or -1)
    $stack = [];
    for ($i = 0; $i < $n; $i++) {
        $tok = $tokens[$i];
        $tv = is_array($tok) ? $tok[1] : $tok;
        if ($tv === '[' || $tv === '{' || $tv === '(') {
            $enclosingOf[$i] = empty($stack) ? -1 : end($stack);
            $stack[] = $i;
        } elseif ($tv === ']' || $tv === '}' || $tv === ')') {
            $openIdx = array_pop($stack);
            $enclosingOf[$i] = ($openIdx !== null && !empty($stack)) ? end($stack) : -1;
            if ($openIdx !== null) {
                $matchOf[$openIdx] = $i;
            }
        } else {
    ...
    // ---- Step 2: find every 'text' => value  (or 'text' : value) pair ----
    $buttonKeys = ['url', 'callback_data', 'web_app', 'request_contact', 'request_location', 'switch_inline_query', 'pay', 'login_url', 'copy_text'];
    $insertions = [];
    $counter = 0;
    ...
    for ($i = 0; $i < $n; $i++) {
        ...
        $rawVal = $tok[1];
        if (strlen($rawVal) < 2 || substr($rawVal, 1, -1) !== 'text') { continue; }
        ...
        // Locate the array/arg-list that directly encloses this 'text' key.
        $enclosing = $enclosingOf[$i] ?? -1;
        if ($enclosing === -1 || !isset($matchOf[$enclosing])) { continue; }
        $closeIdx = $matchOf[$enclosing];

        // Scan EVERY sibling token inside that same bracket (both
        // before and after 'text') for a button-hint key or an
        // already-present 'style' key.
        $hasButtonHint = false;
        $hasStyle = false;
        for ($p = $enclosing + 1; $p < $closeIdx; $p++) {
            if ($p === $i) { continue; }
            if (($enclosingOf[$p] ?? -2) !== $enclosing) { continue; } // stay at this bracket's own depth
            $t = $tokens[$p];
        ...
        if (!$hasButtonHint || $hasStyle) { continue; }

        $sequence = ['danger', 'success', 'primary'];
        $style = $sequence[$counter % count($sequence)];
        $counter++;
        $insertions[$k] = ",\n    \"style\" $arrowStr \"$style\"";
    }
